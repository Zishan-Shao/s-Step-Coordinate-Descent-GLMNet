# This one has exactly the same solution as the naive updatees, while the naive updates are much faster
...
updateBeta <- function(A, b, lambda, alpha, Beta, m, n, tol, maxit) {
  for (i in 1:maxit) {
    ...
    #if (norm(Beta - prevBeta, type = "1") < tol) {
    #  print("now break")
    #  break # Breaks the loop if change in Beta is below the threshold
    #}
...
alpha <- 0.5
lambda <- 0.02
Beta <- rep(0, ncol(X))  # Beta value can significantly influence the output, maybe because of the newton method's property?
...
a0 = 0 # this is the intercept, to agree with the Stanford glmnet, it will be kept as 0


Beta <- updateBeta(X_train, y_train, lambda, alpha, Beta, length(y_train), ncol(X), 1e-16, 3000)
